...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("PyFortran90Namelists")
   ```

3. Run

   ```julia-repl
   julia> using PyFortran90Namelists
   ```
...
version of PyFortran90Namelists, type
...
```@repl
using Pkg
Pkg.update("PyFortran90Namelists")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall PyFortran90Namelists
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("PyFortran90Namelists")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall PyFortran90Namelists.
